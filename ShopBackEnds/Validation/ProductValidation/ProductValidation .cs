...
namespace ShopBackEnd.Validation.ProductValidation
{
    public class ProductValidation : AbstractValidator<ProductDto>
    {
        ...
        {
            RuleFor(p => p.Id)
                .GreaterThan(0).WithMessage("ProductId must be greater than 0 for update operations.");
            ...
            RuleFor(p => p.LastModifiedDate)
                .NotEmpty().WithMessage("LastModifiedDate is required.")
                .LessThanOrEqualTo(DateTime.Now).WithMessage("LastModifiedDate cannot be in the future.");
        }
    }
}
