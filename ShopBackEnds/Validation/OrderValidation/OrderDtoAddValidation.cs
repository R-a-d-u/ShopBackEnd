...
{
    public class OrderDtoAddValidation : AbstractValidator<OrderDtoAdd>
    {
        public OrderDtoAddValidation()
        {
            RuleFor(o => o.UserId)
                .GreaterThan(0).WithMessage("UserId must be a positive number.");

            RuleFor(o => o.OrderCreatedDate)
                .NotNull().WithMessage("OrderCreatedDate is required.")
                .GreaterThan(DateTime.MinValue).WithMessage("OrderCreatedDate must be a valid date.");
